...
export default function ImportPage() {
  ...
  const saveRow = async (row: ImportRow) => {
    ...
    try {
      const { data:dup } = await sb.from("businesses").select("id")
        .ilike("name", row.name.trim()).maybeSingle();
      if (dup) { patch(row._id,{_status:"duplicate",_error:"Already in database"}); return; }
      ...
      patch(row._id, { _status:"error", _error: e instanceof Error ? e.message : "Error" });
    }
  };

  const saveAll = async () => {
    setBusy(true);
    for (const row of rows.filter(r => r._status==="pending")) await saveRow(row);
    setBusy(false);
  };
    ...
    <div style={{ paddingBottom:60 }}>
      ...
      <div style={{ marginBottom:22 }}>
        ...
        <p style={{ color:"var(--text-muted)", fontSize:"0.875rem", marginTop:5 }}>
          Supports both raw Google Maps text and labelled format — paste either and every field fills automatically.
        </p>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14, marginBottom:22 }}>
        ...
        <div style={{ background:"rgba(99,102,241,0.06)", border:"1px solid rgba(99,102,241,0.18)", borderRadius:14, padding:"14px 16px" }}>
          <p style={{ fontWeight:700, color:"#818cf8", fontSize:"0.8rem", marginBottom:8 }}>✅ Format A — Labelled (what you use)</p>
          <pre style={{ fontFamily:"monospace", fontSize:"0.72rem", color:"var(--text-muted)", lineHeight:1.7, margin:0, whiteSpace:"pre-wrap" }}>
{`Name: Chizzychops & Grillz
Category: Restaurant
...
Hours: Open, closes 8 PM`}
        ...
        {/* Format B */}
        <div style={{ background:"rgba(99,102,241,0.06)", border:"1px solid rgba(99,102,241,0.18)", borderRadius:14, padding:"14px 16px" }}>
          <p style={{ fontWeight:700, color:"#818cf8", fontSize:"0.8rem", marginBottom:8 }}>✅ Format B — Raw Maps text (Ctrl+A copy)</p>
          <pre style={{ fontFamily:"monospace", fontSize:"0.72rem", color:"var(--text-muted)", lineHeight:1.7, margin:0, whiteSpace:"pre-wrap" }}>
{`Elegance Salon & Spa
4.8 (312 reviews)
Beauty salon
14 Admiralty Way, Lekki, Lagos
+234 803 123 4567
Open · Closes 8 pm`}
          </pre>
        </div>
      ...
      {stage === "paste" && (
        <div className="glass" style={{ borderRadius:16, padding:22 }}>
          ...
          <textarea
            ...
            rows={14}
            placeholder={
              "Paste in either format:\n\n" +
              "Name: Chizzychops & Grillz\n" +
              "Category: Restaurant\n" +
              "Rating: 4.9 (34 reviews)\n" +
              "Phone: 0809 494 6923\n" +
              "Address: Santos Estate Akowonjo, Lagos\n" +
              "Hours: Open, closes 8 PM\n\n" +
              "--- or ---\n\n" +
              "Elegance Salon\n" +
              "4.8 (312 reviews)\n" +
            ...
            style={{
              ...
              borderRadius:12, color:"var(--text)", fontSize:"0.82rem",
              fontFamily:"monospace", lineHeight:1.8, outline:"none",
          ...
          <div style={{ display:"flex", gap:10, marginTop:14, alignItems:"center" }}>
            ...
            <button className="btn-ghost" onClick={() => setRaw("")} disabled={!raw}>
              Clear
            </button>
            <span style={{ marginLeft:"auto", fontSize:"0.74rem", color:"var(--text-muted)" }}>
              Multiple businesses? Separate with a blank line
            </span>
          </div>
        </div>
      )}
        ...
        <>
          ...
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12, marginBottom:14 }}>
            <div style={{ display:"flex", gap:7, flexWrap:"wrap", alignItems:"center" }}>
              <span style={pill("rgba(255,255,255,0.06)", "var(--text-muted)", "var(--border)")}>
                <strong style={{ color:"#fff" }}>{counts.total}</strong>&nbsp;businesses
              </span>
            ...
            <div style={{ display:"flex", gap:8 }}>
              ...
              <button className="btn-ghost" onClick={() => setStage("paste")}
              ...
              <button className="btn-primary" onClick={saveAll} disabled={busy || counts.pending===0}>
                {busy ? "Saving…" : `💾 Save All to DB (${counts.pending})`}
              </button>
            </div>
          </div>
          ...
          <div style={{ marginBottom:12, padding:"10px 14px", borderRadius:10, background:"rgba(99,102,241,0.05)", border:"1px solid rgba(99,102,241,0.15)", fontSize:"0.78rem", color:"var(--text-muted)" }}>
            💡 Click any cell to edit it before saving. Empty cells were not found in your paste.
          </div>
                      ...
                      <tr key={row._id} style={{
                        ...
                        {COLS.map(col => (
                          <td key={col.key} style={{ padding:"7px 6px", verticalAlign:"middle" }}>
                            {isSaved ? (
                              <span style={{
                                display:"block", padding:"5px 8px", fontSize:"0.775rem",
                                color:row[col.key] ? "var(--text)" : "#44445a",
                                overflow:"hidden", textOverflow:"ellipsis",
                                whiteSpace:"nowrap", maxWidth:col.width-16,
                              }} title={row[col.key]}>
                                {col.key==="rating" && row.rating
                                  ? `${row.rating} ★`
                                  : (row[col.key] || "—")}
                              </span>
                            ) : (
                              <EditCell
                                value={col.key==="rating" && row.rating
                                  ? `${row.rating} ★`
                                  : row[col.key]}
                                onChange={v => {
                                  const clean = col.key==="rating"
                                    ? v.replace(/\s*★+\s*$/, "").trim()
                                    : v;
                                  updateCell(row._id, col.key, clean);
                                }}
                                width={col.width}
                                mono={col.mono}
                              />
                            ...
                            {(row._status==="pending" || row._status==="error") && (
                              <button
                                ...
                                style={{
                                  ...
                                  border:"1px solid rgba(99,102,241,0.28)", transition:"all 0.15s",
                                }}
