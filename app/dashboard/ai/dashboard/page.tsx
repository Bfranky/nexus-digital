...
import { formatMoney, timeAgo, STATUS_META } from "@/lib/utils";
...
function StatCard({ label, value, accent }: { label:string; value:string; accent:string }) {
  return (
    <div className="stat-card">
      <div style={{ fontFamily:"var(--font-display)", fontWeight:700, fontSize:"1.6rem", color:accent }}>{value}</div>
...
export default function DashboardPage() {
  const { businesses, set, setLoading, loading, stats } = useStore();
  ...
  return (
    <div>
      ...
      {!loading && (
        <>
          {/* Stats grid — 2 cols mobile, 3 mid, 6 desktop */}
          <div className="grid-stats" style={{ marginBottom:20 }}>
            <StatCard label="Total"    value={String(s.total)}           accent="#818cf8" />
            <StatCard label="Paid"     value={String(s.paid)}            accent="#4ade80" />
            <StatCard label="Done"     value={String(s.completed)}       accent="#4ade80" />
            <StatCard label="Revenue"  value={formatMoney(s.revenue)}    accent="#4ade80" />
            <StatCard label="Pipeline" value={formatMoney(s.pending)}    accent="#fbbf24" />
          ...
          <div className="detail-layout">
            ...
            <div className="glass" style={{ borderRadius:16, overflow:"hidden" }}>
              ...
              {recent.length === 0 ? (
                <div style={{ padding:"40px 18px", textAlign:"center", color:"var(--text-muted)", fontSize:"0.875rem" }}>
                  No businesses yet. <Link href="/dashboard/import" style={{ color:"#818cf8" }}>Import some →</Link>
                </div>
              ) : recent.map(b => (
                <Link key={b.id} href={`/dashboard/businesses/${b.id}`}
                  ...
                  <div style={{ width:34, height:34, borderRadius:10, background:"linear-gradient(135deg,rgba(99,102,241,0.25),rgba(139,92,246,0.25))", display:"flex", alignItems:"center", justifyContent:"center", fontWeight:700, fontSize:"0.85rem", color:"#fff", flexShrink:0 }}>
                    {b.name.charAt(0)}
                  </div>
            ...
            <div className="glass" style={{ borderRadius:16, overflow:"hidden" }}>
              <div style={{ padding:"14px 18px", borderBottom:"1px solid var(--border)" }}>
                <span style={{ fontFamily:"var(--font-display)", fontWeight:700, color:"#fff", fontSize:"0.95rem" }}>🔔 Follow-ups</span>
              </div>
              {followUps.length === 0 ? (
                <div style={{ padding:"32px 18px", textAlign:"center", color:"var(--text-muted)", fontSize:"0.82rem" }}>All caught up!</div>
              ) : followUps.map(b => (
                <Link key={b.id} href={`/dashboard/businesses/${b.id}`}
                  style={{ display:"flex", alignItems:"center", gap:10, padding:"11px 18px", borderBottom:"1px solid var(--border)", textDecoration:"none" }}>
