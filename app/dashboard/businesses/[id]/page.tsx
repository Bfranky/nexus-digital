...
export default function BusinessDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!biz)    return <div style={{ padding: "60px 0", textAlign: "center", color: "var(--text-muted)" }}>Business not found.</div>;

  return (
    <div style={{ maxWidth: 960 }}>
      {/* Back */}
      <div style={{ marginBottom: 20 }}>
        <Link href="/dashboard/businesses" style={{ color: "var(--text-muted)", textDecoration: "none", fontSize: "0.875rem" }}>
          ← All Businesses
        </Link>
      </div>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          }}>{biz.name.charAt(0)}</div>
          <div>
            <h1 style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: "1.4rem", color: "#fff" }}>{biz.name}</h1>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {saving && <span style={{ fontSize: "0.8rem", color: "var(--text-muted)", alignSelf: "center" }}>Saving…</span>}
          <button className="btn-ghost" style={{ color: "#f87171", borderColor: "rgba(239,68,68,0.25)" }} onClick={deleteBusinesss}>
            🗑 Delete
          </button>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: 20, alignItems: "start" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div className="glass" style={{ borderRadius: 16, padding: 20 }}>
            <h2 style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: "0.95rem", color: "#fff", marginBottom: 16 }}>Contact</h2>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              {[
                ...
                <div key={label}>
                  <div style={{ fontSize: "0.7rem", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em", color: "var(--text-muted)", marginBottom: 4 }}>{label}</div>
                  {value ? (
                    href ? <a href={href} target="_blank" rel="noreferrer" style={{ color: "#818cf8", fontSize: "0.875rem", textDecoration: "none" }}>{value}</a>
          ...
          <div className="glass" style={{ borderRadius: 16, padding: 20 }}>
            ...
            {editNotes ? (
              ...
              <p style={{ fontSize: "0.875rem", color: biz.notes ? "var(--text)" : "var(--text-muted)", lineHeight: 1.65 }}>
          ...
          {/* Project */}
          <div className="glass" style={{ borderRadius: 16, padding: 20 }}>
            <h2 style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: "0.95rem", color: "#fff", marginBottom: 16 }}>Project</h2>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ height: 8, background: "rgba(255,255,255,0.08)", borderRadius: 99, overflow: "hidden" }}>
                <div style={{
                  ...
                  width: `${biz.project_progress}%`,
                  background: biz.project_progress === 100 ? "#22c55e" : "#6366f1",
                }} />
              </div>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              <div style={{ flex: 1, minWidth: 180 }}>
                <label style={{ fontSize: "0.7rem", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em", color: "var(--text-muted)", display: "block", marginBottom: 6 }}>Demo URL</label>
                <input className="field" defaultValue={biz.project_demo_url ?? ""} placeholder="https://…"
                  onBlur={e => patch({ project_demo_url: e.target.value || null })} />
              </div>
              <div style={{ flex: 1, minWidth: 180 }}>
                <label style={{ fontSize: "0.7rem", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em", color: "var(--text-muted)", display: "block", marginBottom: 6 }}>Live URL</label>
                <input className="field" defaultValue={biz.project_live_url ?? ""} placeholder="https://…"
                  onBlur={e => patch({ project_live_url: e.target.value || null })} />
              </div>
            </div>

            {/* Services */}
            <div style={{ marginTop: 16 }}>
              <label style={{ fontSize: "0.7rem", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em", color: "var(--text-muted)", display: "block", marginBottom: 10 }}>Services Requested</label>
                ...
                {SERVICES.map(s => {
                  ...
                  return (
                    <button key={s} onClick={() => {
                      const next = active
                        ? biz.services_requested.filter(x => x !== s)
                        : [...biz.services_requested, s];
          ...
          {/* Payment */}
          <div className="glass" style={{ borderRadius: 16, padding: 20 }}>
            <h2 style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: "0.95rem", color: "#fff", marginBottom: 16 }}>Payment</h2>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <div>
                <label style={{ fontSize: "0.7rem", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em", color: "var(--text-muted)", display: "block", marginBottom: 6 }}>Quoted (₦)</label>
              ...
              <div>
                <label style={{ fontSize: "0.7rem", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em", color: "var(--text-muted)", display: "block", marginBottom: 6 }}>Paid (₦)</label>
              ...
              <div style={{ gridColumn: "1/-1" }}>
                <label style={{ fontSize: "0.7rem", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em", color: "var(--text-muted)", display: "block", marginBottom: 6 }}>Payment Date</label>
                <input className="field" type="date" defaultValue={biz.payment_date ?? ""}
                  onBlur={e => patch({ payment_date: e.target.value || null })} />
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div className="glass" style={{ borderRadius: 16, padding: 16 }}>
            <h2 style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: "0.9rem", color: "#fff", marginBottom: 12 }}>Pipeline</h2>
            <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              {STATUS_ORDER.map((s, i) => {
                const currentIdx = STATUS_ORDER.indexOf(biz.status);
                const isCurrent  = s === biz.status;
                ...
                return (
                  <button key={s} onClick={() => patch({ status: s })}
                    ...
                    <span style={{ width: 6, height: 6, borderRadius: "50%", flexShrink: 0,
                      background: isCurrent ? "#818cf8" : isPast ? "#4ade80" : "rgba(255,255,255,0.15)" }} />
                    {STATUS_META[s].label}
                  </button>
                );
              })}
          ...
          <div className="glass" style={{ borderRadius: 16, padding: 16 }}>
            ...
            <label style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer", fontSize: "0.875rem", color: "var(--text)" }}>
              <input type="checkbox" checked={biz.whatsapp_replied}
                onChange={e => patch({ whatsapp_replied: e.target.checked })}
                style={{ accentColor: "#6366f1", width: 16, height: 16 }} />
              Replied on WhatsApp
            </label>
            {biz.whatsapp && (
              <a href={`https://wa.me/${biz.whatsapp.replace(/\D/g,"")}`} target="_blank" rel="noreferrer"
                className="btn-primary" style={{ textDecoration: "none", marginTop: 12, display: "inline-flex", padding: "8px 16px", fontSize: "0.8rem", background: "#25d366" }}>
                💬 Open WhatsApp
              </a>
            )}
          </div>
          ...
          <div className="glass" style={{ borderRadius: 16, padding: 16 }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 10, fontSize: "0.8rem" }}>
              <div>
                ...
                <br /><span style={{ color: "var(--text)" }}>{formatDate(biz.created_at)}</span>
              </div>
              <div>
                <span style={{ color: "var(--text-muted)" }}>Updated</span>
                <br /><span style={{ color: "var(--text)" }}>{timeAgo(biz.updated_at)}</span>
              </div>
              {biz.google_maps_url && (
                <a href={biz.google_maps_url} target="_blank" rel="noreferrer" style={{ color: "#818cf8", textDecoration: "none" }}>
                  🗺 View on Maps →
                </a>
              )}
            </div>
